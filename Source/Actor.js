...
class Actor2 {
    constructor(color, pos, activity, items) {
        ...
        this.firePolar = new Polar((this.azimuthInTurnsMin + this.azimuthInTurnsMax) / 2, this.collider.radius * 2, 0);
        this.muzzlePos = pos.clone().add(this.firePolar.toCoords(Coords.create()));
        this.reset();
    }
    firePolarAbsolute() {
    ...
    updateForTimerTick(world) {
        if (this.ticksSinceKilled == null) {
            ...
            var surfaceAltitude = landscape.altitudeAtX(pos.x);
    ...
    drawToDisplay(display) {
        ...
        display.drawWedge(pos, this.collider.radius, angleStart, angleStop, this.color, null // colorBorder
        );
        var world = Globals.Instance().world;
        if (this == world.actorCurrent()) {
            var fireAzimuthInTurnsRecentered = Math.abs(0.75 - this.firePolar.azimuthInTurns);
            var fireAzimuthInDegrees = Math.round(fireAzimuthInTurnsRecentered
            ...
            var text = "Weapon:" + this.itemSelected().toString()
