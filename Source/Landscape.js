...
class Landscape {
    ...
    altitudeAtX(xToCheck) {
        ...
        var horizonChange = horizonPoint.clone().subtract(horizonPointPrev);
        var t = (xToCheck - horizonPointPrev.x)
            / (horizonChange.x);
        var altitude = horizonPointPrev.y
            + (t * horizonChange.y);
        return altitude;
    }
    collidesWithEdge(edgeOther) {
        ...
        for (var i = 0; i < this.edges.length; i++) {
            ...
            var doEdgesCollide = collisionHelper.doEdgesCollide(edgeThis, edgeOther);
            if (doEdgesCollide) {
                returnValue = true;
                break;
            }
    ...
    slopeAtX(xToCheck) {
        ...
        var horizonPointPrev = edge.vertices[0];
        var horizonPoint = edge.vertices[1];
        var horizonChange = horizonPoint.clone().subtract(horizonPointPrev).normalize();
